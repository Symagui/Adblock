...
\code{
  struct header {
    char ssl; //1 si c'est une connexion HTTPS
    char method[500]; //GET, CONNECT, POST...
    char path[500]; //Chemin demandé
    char protocol[500]; //HTTP/1.1 par exemple
    char hostname[500]; //Nom du domaine
    struct host hst; //Données calculées selon le nom de domaine pour obtenir une adresse réelle sur le réseau.
  };
  struct host {
    int port; //Port
    int addr_family; //Type d'adresse (AF_INET pour IPv4, AF_INET6 pour IPv6)
    int addr_len;
